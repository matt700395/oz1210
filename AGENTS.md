...
> **중요**: 2025년 4월 1일부터 Clerk의 Supabase JWT 템플릿이 deprecated되었습니다. 네이티브 통합 방식을 사용해야 합니다.

#### 설정 단계

1. **Clerk Dashboard에서 Supabase 통합 활성화**:
   - [Clerk Dashboard](https://dashboard.clerk.com/setup/supabase) → Supabase 통합 설정
   - "Activate Supabase integration" 클릭
   - Clerk domain 복사 (예: `your-app-12.clerk.accounts.dev`)

2. **Supabase Dashboard에서 Clerk를 Third-Party Auth Provider로 추가**:
   - Supabase Dashboard → Settings → Authentication → Providers
   - "Third-Party Auth" 섹션에서 "Add provider" → "Clerk" 선택
   - Clerk domain 입력 후 저장

3. **로컬 개발 설정** (선택사항):
   - `supabase/config.toml`의 `[auth.third_party.clerk]` 섹션에서 `enabled = true` 및 `domain` 설정
   - 로컬 Supabase CLI 사용 시에만 필요
...
3. **Supabase 클라이언트가 Clerk 토큰을 사용하여 인증** (JWT 템플릿 불필요)
   - `accessToken` 함수를 통해 Clerk 세션 토큰을 자동으로 전달
   - Supabase가 Clerk 토큰을 검증하여 `auth.jwt()->>'sub'`로 Clerk user ID 접근 가능

#### Supabase 클라이언트 파일들 (`lib/supabase/`)

- **`server.ts`**: Server Component/Server Action용
  - **`createClient()`**: 공식 Next.js 가이드 패턴 (쿠키 기반, Clerk 없이 사용 가능)
    - `@supabase/ssr`의 `createServerClient` 사용
    - Supabase 공식 Next.js 퀵스타트 가이드 패턴
    - `await createClient()` 형태로 사용
    - 예제: `app/instruments/page.tsx`
  - **`createClerkSupabaseClient()`**: Clerk 통합용
...
- **`clerk-client.ts`**: Client Component용 (`useClerkSupabaseClient` hook)
  ...
  - `accessToken` 옵션 사용 시 `supabase.auth`는 사용 불가 (Clerk 훅 사용)
...
- RLS 정책 예제: `supabase/migrations/20250101000000_example_rls_policies.sql`
- 사용 예제 코드: `docs/clerk-supabase-integration-examples.md`
- RLS 정책은 `auth.jwt()->>'sub'`로 Clerk user ID를 확인하여 사용자별 데이터 접근 제어
- 개발 중에는 RLS를 비활성화할 수 있으나, 프로덕션에서는 반드시 활성화 필요
...
- `lib/supabase/server.ts`: Supabase 클라이언트 (공식 가이드 패턴 + Clerk 통합)
- `app/instruments/page.tsx`: Supabase 공식 가이드 예제 페이지
- `components.json`: shadcn/ui 설정

## Supabase 공식 가이드 참고

이 프로젝트는 [Supabase 공식 Next.js 퀵스타트 가이드](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)의 패턴을 따릅니다:

- `@supabase/ssr` 패키지 사용
- `createServerClient`를 통한 쿠키 기반 인증
- Server Component에서 `await createClient()` 패턴
- 예제: `app/instruments/page.tsx` 참고
