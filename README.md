...
5. **3-1**에서 복사한 **Clerk domain** 붙여넣기
...
**6-3-1. Supabase 기본 연결 테스트**

공식 Next.js 가이드 패턴을 사용하여 Supabase 연결을 테스트할 수 있습니다:

1. Supabase Dashboard → **SQL Editor**에서 다음 SQL 실행:
   ```sql
   -- instruments 테이블 생성 (공식 가이드 예제)
...
2. 개발 서버 실행 후 `http://localhost:3000/instruments` 접속하여 데이터 확인

> **참고**: 공식 가이드 예제는 [Supabase Next.js 퀵스타트](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)를 참고하세요.
...
- 로그인, 회원가입, 프로필 관리 등 모든 UI가 한국어로 제공됩니다
- 커스텀 에러 메시지도 설정 가능합니다

**자세한 내용**: [Clerk 한국어 로컬라이제이션 가이드](./docs/clerk-korean-localization.md) 참고

> **참고**: Clerk Account Portal은 여전히 영어로 표시됩니다. 이는 Clerk의 호스팅된 관리 페이지이므로 변경할 수 없습니다.
