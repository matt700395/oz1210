...
- [x] API 클라이언트 구현
  - [x] `lib/api/tour-api.ts` 생성
    ...
    - [x] 에러 처리 및 재시도 로직
  ***
  - [x] 타입 정의 파일 생성 (`lib/types/tour.ts`)
    - [x] 기본 인터페이스 정의 (TourItem, TourDetail, TourIntro, TourImage, PetTourInfo)
    - [x] API 응답 래퍼 타입 정의 (TourApiResponse, TourApiResponseHeader, TourApiResponseBody)
    - [x] API 요청 파라미터 타입 정의 (AreaCodeParams, AreaBasedListParams 등)
    - [x] 유틸리티 타입 정의 (ContentTypeId, TourListResponse)
    - [x] 커스텀 에러 클래스 (TourApiError)
  - [x] 공통 유틸리티 함수 구현
    - [x] `getApiKey()` - 환경변수에서 API 키 조회 (NEXT_PUBLIC_TOUR_API_KEY 우선, TOUR_API_KEY 대체)
    - [x] `buildQueryString()` - URL 쿼리 문자열 생성 (공통 파라미터 자동 포함)
    - [x] `parseApiResponse()` - API 응답 파싱 (response.body.items.item 구조 처리)
  - [x] 재시도 로직 구현
    - [x] `fetchWithRetry()` - 재시도 로직 포함 fetch 함수
    - [x] 지수 백오프 적용 (1초, 2초, 4초)
    - [x] 네트워크 에러 및 HTTP 5xx 에러 재시도
    - [x] 최대 재시도 횟수 (3회) 설정
- [x] 타입 정의
...
- [x] 레이아웃 구조
  ...
  - [x] `components/Navbar.tsx` 업데이트
    ...
    - [x] 네비게이션 링크 (홈, 통계, 북마크)
  ***
  - [x] 통계 타입 정의 파일 생성 (`lib/types/stats.ts`)
    - [x] `RegionStats` 인터페이스 정의 (code, name, count)
    - [x] `TypeStats` 인터페이스 정의 (typeId, typeName, count, percentage)
    - [x] `StatsSummary` 인터페이스 정의 (totalCount, topRegions, topTypes, lastUpdated)
    - [x] `CONTENT_TYPE_NAMES` 상수 정의 (관광 타입 ID와 한글명 매핑)
  - [x] 레이아웃 메타데이터 업데이트
    - [x] title 변경: "My Trip - 한국 관광지 정보 서비스"
    - [x] description 변경: "전국 관광지 정보를 검색하고 지도에서 확인하는 서비스"
    - [x] Open Graph 태그 추가
    - [x] Twitter Card 태그 추가
  - [x] Navbar 컴포넌트 업데이트
    - [x] 로고 텍스트 변경 (SaaS Template → My Trip)
    - [x] 검색창 UI 추가 (데스크톱, 기능은 Phase 2에서 구현)
    - [x] 네비게이션 링크 추가 (홈, 통계, 북마크)
    - [x] 데스크톱 레이아웃: 로고 + 네비게이션 + 검색창 + 로그인 버튼
